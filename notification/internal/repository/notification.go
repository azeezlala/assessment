...
type NotificationRepository interface {
	AddNotification(ctx context.Context, userID, message, typ string) error
...
func (s *NotificationObj) AddNotification(ctx context.Context, id, message, typ string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	s.notifications[id] = append(s.notifications[id], notification)
	return nil
}
